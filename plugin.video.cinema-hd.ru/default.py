...
def mainScreen(params):
    ...
    for n in content: 
        ...
        listitem=xbmcgui.ListItem(title.replace('<h2>','').replace('</h2>',''),iconImage = addon_icon, thumbnailImage = pic)
        listitem.setInfo(type='video', infoLabels = {'plot':desk,'plotoutline':desk})
        uri = construct_request({
            'func': 'get_movies',
            'url':url,
...
def get_movies(params):
    ...
    txt=str(beautifulSoup).replace(' онлайн в хорошем качестве HD 720p','').replace(' онлайн в хорошем качестве','').replace('<font>Смотреть фильм ','').replace('<font>Смотреть сериал ','').replace('Смотреть онлайн фильм в хорошем качестве HD 720p','').replace('<font><font>','<font>').replace('<font></font>','').replace('<font> </font>','').replace('</font></font>','</font>').replace('<h2>','').replace('</h2>','')
    ...
    for n in m: 
        ...
        listitem=xbmcgui.ListItem(n[0].split("</font>")[0].replace('смотреть онлайн','').replace('<h2>','').replace('</h2>',''),iconImage = addon_icon, thumbnailImage = params['img'])
        uri = construct_request({
            'func': 'get_movie',
            'url':n[1].replace('amp;','')
            })
    ...
    if not m: 
        ...
        for n in m:
            listitem=xbmcgui.ListItem(params['title'].replace('смотреть онлайн','').replace('<h2>','').replace('</h2>',''),iconImage = addon_icon, thumbnailImage = params['img'])
            uri = construct_request({
                'func': 'get_movie',
                'url':n.replace('amp;','')
                })
