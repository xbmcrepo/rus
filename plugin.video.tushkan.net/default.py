...
def doSearch(params):
    try: 
        http = GET("http://tushkan.net"+params['link'])
    except: 
...
def openitem(params):
    http = GET("http://tushkan.net"+params['mpath'])
    beautifulSoup = BeautifulSoup(http)
    #print http
    ...
    if tttl:
        ...
        link=tttl[0]
        http = GET(link)
        link=re.findall("pl:(.+), ",str(http))[0].replace("'",'"').replace(', st:"uppodvideo"})','')
        if link:
...
def _doSearch(params):
    ...
    if kbd.isConfirmed():
        sts=kbd.getText()
        params={}
        params['link']='http://tushkan.net/search/?q=%s&sfSbm=&a=45'%urllib.quote(sts)
        params['mode']='link'
        doSearch(params)
